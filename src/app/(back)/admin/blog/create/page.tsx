...
import { createBlogPostAction, uploadBlogImageAction, getBlogCategoriesAction } from "@/app/actions/blog";
import RichTextEditor from "@/components/ui/rich-text-editor";
import SafeImage from "@/components/ui/safe-image";
import { FileText, ArrowLeft, Send, Upload, Loader2, Image as ImageIcon } from "lucide-react";
    ...
    <div className="max-w-4xl mx-auto space-y-8 font-sans pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-6">
        <div>
          ...
          >
            <ArrowLeft className="w-4 h-4" /> Return to Blog List
          </Link>
          <h1 className="text-3xl font-black text-brand-navy">Create New Blog Post</h1>
          <p className="text-sm text-brand-slate mt-1 font-medium">
            Publish CQC compliance guides, staffing advice, and healthcare insights directly to your website.
          </p>
        </div>
      </div>

      {/* DEDICATED BLOG CREATION FORM */}
      <form onSubmit={handleSubmit} className="bg-white border border-slate-200 rounded-3xl p-6 md:p-8 space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <input
              ...
              placeholder="e.g. Effective CQC Safeguarding Strategies for Care Homes"
        ...
        <div>
          <label className="block text-xs font-bold text-brand-navy mb-2 uppercase tracking-wider">
            Short Excerpt / Summary (for Article Cards & SEO Search Snippets)
          </label>
          <textarea
            required
            rows={3}
            ...
            placeholder="Provide a 2-sentence summary detailing what care managers and readers will learn from this post..."
            className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-3.5 text-sm text-brand-navy focus:outline-none focus:border-brand-navy focus:bg-white font-medium transition-all"
          />
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <label className="block text-xs font-bold text-brand-navy mb-2 uppercase tracking-wider">
              Author Name / Team Tag
            </label>
            <input
              type="text"
              value={authorName}
              onChange={(e) => setAuthorName(e.target.value)}
              placeholder="e.g. New Era Compliance Team"
              className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-3.5 text-sm text-brand-navy focus:outline-none focus:border-brand-navy focus:bg-white font-medium transition-all"
            />
          </div>
        ...
        <div className="space-y-2">
          ...
          <p className="text-xs text-brand-slate">
            Format headings, list items, quotes, inline links, and upload device images directly into the content stream.
          </p>
